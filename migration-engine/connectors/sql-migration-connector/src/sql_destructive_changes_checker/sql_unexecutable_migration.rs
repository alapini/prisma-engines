...
impl Check for SqlUnexecutableMigration {
    ...
    fn check_existing_values(&self) -> Option<(&str, &str)> {
        match self {
            ...
            SqlUnexecutableMigration::AddedRequiredFieldToTable { table, column: _ } => None,
        }
    }

    fn render<'a>(&self, database_checks: &DatabaseCheckResults) -> Option<String> {
        match self {
            SqlUnexecutableMigration::AddedRequiredFieldToTable { table, column } => {
                ...
                let message = match database_checks.rows_in_table(table) {
            ...
            SqlUnexecutableMigration::MadeOptionalFieldRequired { table, column } => {
                match database_checks.values_in_column(table, column) {
                    (Some(0), _) => None,
